/**
 * Next.js API Route: /api/integrations/[...path]
 *
 * Acts as a secure proxy between the browser and the FastAPI backend.
...
export async function PATCH(
  ...
  const resolvedParams = await params;
  return proxyToBackend(request, resolvedParams.path, 'PATCH');
}
...
async function proxyToBackend(
  ...
  const targetUrl = `${BACKEND_URL}/api/v1/integrations/${backendPath}`;
