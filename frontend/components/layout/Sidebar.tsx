...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { name: "Outgoing Box", href: "/outbox", icon: Send },
  ];
  ...
  return (
    ...
    >
      ...
      <div className={`py-4 border-t border-outline-variant bg-surface-container-lowest transition-all duration-300 ${isExpanded ? 'px-4' : 'flex flex-col items-center gap-4'}`}>
          ...
          <div className={`flex items-center ${isExpanded ? 'gap-3' : ''}`}>
            ...
            {isExpanded && (
              <div className="flex flex-col overflow-hidden">
                <span className="font-bold tracking-widest text-on-surface uppercase text-[11px] truncate max-w-[100px]">{userName}</span>
                <span className="text-[9px] text-on-surface-variant font-mono truncate max-w-[100px] uppercase">{userRole}</span>
              </div>
            )}
          </div>

          {/* Action Button (Sign Out) within the pill when expanded */}
          {isExpanded && (
            <button 
              onClick={(e) => { e.stopPropagation(); handleLogout(); }}
              className="w-8 h-8 rounded-full flex items-center justify-center text-on-surface-variant hover:text-red-400 hover:bg-[rgba(248,113,113,0.15)] transition-colors mr-1 opacity-0 group-hover:opacity-100"
              title="Sign Out"
            >
        ...
        {/* If collapsed, logout button stays below */}
        {!isExpanded && (
          <button 
            onClick={handleLogout}
            className="relative group w-10 h-10 rounded-xl flex items-center justify-center text-on-surface-variant hover:text-red-400 hover:bg-[rgba(248,113,113,0.1)] transition-all flex-shrink-0 mt-2"
          >
            <LogOut className="w-4 h-4" />
