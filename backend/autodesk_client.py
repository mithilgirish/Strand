...
class AutodeskClient:
    def get_client_credentials(self):
        ...
        config = redis_client.get_cache("autodesk_config")
    ...
    def exchange_code(self, code: str) -> dict:
        ...
        if settings.DEMO_MODE and (code == "tokenspark" or client_id in ["admin@strand", "tokenspark"]):
            ...
            redis_client.set_cache("autodesk_token", token_data, ttl=86400)
            redis_client.set_cache("autodesk_refresh", "admin-refresh-token-xyz", ttl=86400 * 14)
            return token_data
        ...
        # Save to Redis globally for the tenant (hackathon simplification)
        redis_client.set_cache("autodesk_token", token_data, ttl=token_data.get("expires_in", 3599))
        if "refresh_token" in token_data:
            redis_client.set_cache("autodesk_refresh", token_data["refresh_token"], ttl=86400 * 14)
    ...
    def get_2legged_token(self) -> str:
        ...
        # Check cache first
        token_data = redis_client.get_cache("autodesk_2legged_token")
        ...
        redis_client.set_cache("autodesk_2legged_token", res_data, ttl=res_data.get("expires_in", 3599))
        return res_data["access_token"]

    def get_valid_token(self):
        """Retrieve token from Redis, refreshing if necessary, or fall back to 2-legged."""
        # 1. Check for active 3-legged token
        token_data = redis_client.get_cache("autodesk_token")
        if token_data and "access_token" in token_data:
            return token_data["access_token"]
            
        # 2. Try refresh
        refresh_token = redis_client.get_cache("autodesk_refresh")
        if refresh_token:
            try:
                client_id, client_secret = self.get_client_credentials()
                ...
                if response.status_code == 200:
                    ...
                    redis_client.set_cache("autodesk_token", new_token, ttl=new_token.get("expires_in", 3599))
                    redis_client.set_cache("autodesk_refresh", new_token["refresh_token"], ttl=86400 * 14)
    ...
    def download_file(self, project_id: str, version_id: str, output_path: str) -> str:
