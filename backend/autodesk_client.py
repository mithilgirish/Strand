...
class AutodeskClient:
    ...
    def download_file(self, project_id: str, version_id: str, output_path: str, tenant_id: str = None) -> str:
        ...
        if item_response.status_code != 200:
            raise HTTPException(
                status_code=item_response.status_code, detail=f"Failed to fetch version metadata: {item_response.text}"
            )
        ...
        if file_response.status_code != 200:
            raise HTTPException(
                status_code=file_response.status_code, detail=f"Failed to download file: {file_response.text}"
            )
